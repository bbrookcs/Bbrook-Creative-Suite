...
export const actions = {
    ...
    updateStatus: async ({ request }) => {
        ...
        let updateQuery = `UPDATE self_plans SET status = ? WHERE id = ?`;
        if (status === 'Achieved') {
            updateQuery = `UPDATE self_plans SET status = ?, achieved_date = CURRENT_TIMESTAMP WHERE id = ?`;
        }
    ...
    delete: async ({ request }) => {
        ...
        await db.query(`DELETE FROM self_plans WHERE id = ?`, [id]);
        
        await db.query(
            `INSERT INTO self_timeline (action, module, details) VALUES (?, ?, ?)`,
            ['Deleted Plan', 'Plans', JSON.stringify({ id })]
        );
